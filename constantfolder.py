...
#main function
def folder(threeaddrCode, symbolTable, changed):
    threeaddrCode, potentialChange = _constantFoldingMainLoop(threeaddrCode, symbolTable)
...
def _constantFoldingMainLoop(threeAddrCode, symbolTable):
    changed = False
    for scope in threeAddrCode:
        if isinstance(threeAddrCode[scope], dict): #Want to make sure we ignore global variables
            for block in threeAddrCode[scope]:
                threeAddrCode[scope][block], changed = _constantFoldingBlock(threeAddrCode[scope][block], changed, symbolTable, scope)

    return threeAddrCode, changed

#Looks for blocks that we can perform constant folding on
def _constantFoldingBlock(block, changed, symbolTable, scope):
    newBlock = {}
    for index, stmt in block.items():
        if (stmt[-1] == 'decl' or stmt[-1] == 'assign') and len(stmt) == 5: #Only lines with 5 length can be constant folded since it will be a constant, operator, constant
            stmt, potentialChange = _checkingForConstants(stmt, symbolTable, scope)
            newBlock[index] = stmt
...
def _checkingForConstants(stmt, symbolTable, scope):
    if re.match(cc.numbers, stmt[1]) and re.match(cc.numbers, stmt[3]):
        ...
        if evaluatedNumber != False:
            ...
            if stmt[0] in variables:
                varType = symbolTable.get_type(stmt[0], scope)
                if varType[0] == 'int':
                    stmt[1] = str(int(float(evaluatedNumber))) #We floor the number if it is an int
